...
bus = SMBus(3)
file = open("/home/pi/Junk_code/datafiles/"+str(datetime.today().strftime('%Y-%m-%d_%H:%M:%S')) + ".csv", "w+")
...
# Get the H3L goin
bus.write_byte_data(0x18, 0x20, 0x3F) # Turn the chip on, accel enabled, 1000Hz sample
...
while(1):
    ...
    highAccel_data = bus.read_i2c_block_data(0x18, 0xA8, 6)
